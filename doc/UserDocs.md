# Overview
Sudoku solver allows you to generate unlimited number of sudoku puzzles for you to solve, based on the level of techniques you know, so you can solve the sudoku comfortably (or not). It is continuation of my work from the first semester. It includes all the features as before and on top of those Graphical interface and random transformations of the sudoku puzzle.

# Window Guide

## Overlay

Window consists of two parts. Sudoku view on the bottom and function buttons on top.

## Sudoku Viewer

It shows current currently selected sudoku and operations on it. It can be changed to the previous/next sudoku by corresponding buttons on top left/rigt.

## Buttons

### Generate

Randomly regenerates selected sudoku based on difficulty selected in combobox.

### Solve

Solves currently selected sudoku.

### Print

Saves sudoku to PDF format (A4 2x3 per page).

### Random Transform

Performs random transformations on sudoku that doesn't change solvability (mirror, flip, change digits, change rows, columns)

### Add/Remove

Adds new Sudoku/Removes selected Sudoku.
